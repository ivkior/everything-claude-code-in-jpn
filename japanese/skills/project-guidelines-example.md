...
これはプロジェクト固有スキルの例です。自分のプロジェクト用のテンプレートとして使用してください。
...
設計対象の特定プロジェクトで作業する際にこのスキルを参照。プロジェクトスキルには以下が含まれます:
- アーキテクチャ概要
- ファイル構造
- コードパターン
- テスト要件
...
1. **絵文字禁止** - コード、コメント、ドキュメントで
2. **不変性** - オブジェクトや配列を直接変更しない
3. **TDD** - 実装前にテストを書く
4. **80%カバレッジ** 最低限
5. **多くの小さなファイル** - 200-400行が典型、最大800行
6. **console.log禁止** - 本番コードで
