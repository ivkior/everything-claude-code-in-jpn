...
hooks/useAuth.ts              # フックは'use'プレフィックス付きcamelCase
lib/formatDate.ts             # ユーティリティはcamelCase
types/market.types.ts         # 型は.typesサフィックス付きcamelCase
```
...
**覚えておくこと**: コード品質は交渉の余地がない。明確で保守可能なコードは迅速な開発と自信を持ったリファクタリングを可能にする。
